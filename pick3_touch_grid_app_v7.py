...
def combine_rankings(grid_df: pd.DataFrame, chart_df: pd.DataFrame, bonus_if_in_both: float = 25.0) -> pd.DataFrame:
    ...
    g = grid_df.copy()
    c = chart_df.copy()
    g = g.rename(columns={"score":"grid_score", "method_bucket":"grid_bucket"})
    c = c.rename(columns={"score":"chart_score", "method_bucket":"chart_bucket"})
    merged = pd.merge(g, c, on="box", how="outer", suffixes=("", ""))
    ...
    # choose structure from whichever exists
    merged["structure"] = merged["structure_x"].fillna(merged["structure_y"])
    merged["method"] = merged.apply(lambda r: "Combined" if (r["in_grid"] and r["in_chart"]) else ("Grid" if r["in_grid"] else "Chart"), axis=1)
    ...
    def pick_bucket(row):
        ...
        if row["in_grid"]:
            return f"B:{row.get('grid_bucket','Grid')}"
        return f"C:{row.get('chart_bucket','Chart')}"
    merged["method_bucket"] = merged.apply(pick_bucket, axis=1)

    out_cols = ["box","structure","method","method_bucket","score",
                "grid_score","chart_score","in_grid","in_chart",
                "grid_bucket","chart_bucket"]
