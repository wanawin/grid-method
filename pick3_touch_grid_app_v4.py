...
def apply_percentile_trim(
    ...
    df = df_ranked.copy()
    n = len(df)
    df["rank_pct"] = (df["rank"].astype(float) / float(n)) * 100.0
...
def run_transition_backtest(
    ...
    Returns per-test rows with winner_rank_pct (0 best) and winner_rank (1 best).
    """
    ...
    target_positions = [_positional_index_of(df_state, ix) for ix in target_idx_values]
    ...
    for pos in target_positions:
        ...
        if len(df_target_hist) < need:
            continue

        # Determine seed based on transition mode
        if transition_mode == TRANSITION_SAME_DRAWTIME:
            seed_num = str(df_target_hist["result"].iloc[-1]).zfill(3)
        else:
            seed_num = str(df_hist["result"].iloc[-1]).zfill(3)

        winner_num = str(df_state["result"].iloc[pos]).zfill(3)
        winner_box = box_of(winner_num)

        grid = generate_due_grid_from_history(df_target_hist["result"].astype(str).tolist(), lookback=dg_lookback)
        chart = build_parity_pair_chart(df_target_hist["result"].astype(str).tolist(), lookback=chart_lb)

        grid_rank = generate_grid_rankings(
            seed_num=seed_num,
            grid=grid,
            third_digit_sources=third_digit_sources,
            grid_method_weight=1.0,
        )
        chart_rank = generate_parity_pair_rankings(seed_num=seed_num, chart=chart)

        combined_rank = combine_rankings(grid_rank, chart_rank, bonus_if_in_both=combined_bonus)
        if combined_rank.empty:
            continue

        cr = combined_rank.sort_values(["score", "box"], ascending=[False, True]).reset_index(drop=True)
        match = cr.index[cr["box"] == winner_box]
        if len(match) == 0:
            winner_rank = None
            winner_rank_pct = 1.0
        else:
            winner_rank = int(match[0]) + 1
            winner_rank_pct = float(match[0]) / float(len(cr) - 1) if len(cr) > 1 else 0.0
        ...
        rows.append(
            {
                ...
                "candidate_count": int(len(cr)),
                "winner_rank": winner_rank,
                "winner_rank_pct": winner_rank_pct,
            }
        )
...
with st.sidebar:
    ...
    zone_bin_size = st.selectbox("Zone size (for Select zones)", [5, 10], index=0, disabled=not (use_pct_trim and pct_mode=="Select zones"), key="zone_bin_size")
    ...
    zones_keep = st.multiselect("Zones to keep (0 is best ranks)", options=all_zones, default=st.session_state.get("zones_keep", all_zones[:max(1, len(all_zones)//2)]), disabled=not (use_pct_trim and pct_mode=="Select zones"), key="zones_keep")
...
if stream_mode == "Per draw time":
    ...
    if transition_mode == "Auto (choose best)":
        # Auto mode uses the same Due-Grid windowing you selected above.
        # If the window toggle is OFF, we treat it as 'use all available history' (None).
        due_grid_window = int(drought_window) if use_window else None
        # Parity chart lookback matches the Parity Pair 'Pair strength window' setting.
        chart_lookback = int(pair_strength_window)
