...
def run_transition_backtest(
    ...
    for pos in target_positions:
        ...
        if len(df_target_hist) < max(due_grid_window, chart_lookback, 5):
            continue
        ...
        grid = generate_due_grid_from_history(df_target_hist["result"].astype(str).tolist(), lookback=due_grid_window)
        chart = build_parity_pair_chart(df_target_hist["result"].astype(str).tolist(), lookback=chart_lookback)
...
due_grid_window = st.slider(
    ...
    min_value=3,
    max_value=20,
    value=int(st.session_state.get("due_grid_window", 6)),
    step=1,
    help="Matches the original creator style: for each position, find digits missing from the last N draws (for that stream).",
    key="due_grid_window",
)
